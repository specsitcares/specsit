...
const blank = () => ({ id: null, name: '', link: '', image: null, _file: null });

/**
 * Reusable editor for a homepage section made of cards (name + link + image),
 * backed by /cms/section-cards/?section=<sectionKey> and the HomeSection meta.
 *
 * Props: sectionKey, pageTitle, defaultTitle, nameLabel, addLabel,
 *        tabs (optional [{label, active}]).
 */
const SectionCardsEditor = ({ sectionKey, pageTitle, defaultTitle, nameLabel = 'Card Name', addLabel = 'Add Card', tabs = [] }) => {
    ...
    const load = async () => {
        ...
        try {
            const [secRes, cardRes] = await Promise.all([
                ...
                apiClient.get(`/cms/section-cards/?section=${sectionKey}`, { cache: false }),
            ]);
            const sec = (secRes.data.results || secRes.data || []).find(s => s.key === sectionKey);
            if (sec) { setSection(sec); setTitle(sec.title || defaultTitle || ''); setVisible(sec.is_published); }
            setCards((cardRes.data.results || cardRes.data || []).map(c => ({ ...c, _file: null })));
        } catch { /* toast */ } finally { setLoading(false); }
    };
    useEffect(() => { load(); }, [sectionKey]);

    const setCard = (i, f, v) => setCards(prev => prev.map((c, j) => j === i ? { ...c, [f]: v } : c));
    const addCard = () => setCards(prev => [...prev, blank()]);
    const removeCard = (i) => {
        const c = cards[i];
    ...
    const save = async () => {
        ...
        try {
            if (section) {
                await apiClient.patch(`/cms/home-sections/${section.id}/`, { title, is_published: visible, status: visible ? 'published' : 'draft' });
            }
            for (const id of removed) await apiClient.delete(`/cms/section-cards/${id}/`);
            for (let i = 0; i < cards.length; i++) {
                ...
                if (c.id) await apiClient.patch(`/cms/section-cards/${c.id}/`, fd);
                else await apiClient.post('/cms/section-cards/', fd);
            }
            setRemoved([]);
            await load();
    ...
    return (
        <div style={{ fontFamily: 'Roboto, sans-serif', padding: '24px 26px 90px' }}>
            <div style={{ fontSize: 13, color: '#64748B', marginBottom: 8 }}>
                <span style={{ cursor: 'pointer' }} onClick={() => navigate('/admin/settings/cms')}>Homepage Management</span> &nbsp;›&nbsp; <span style={{ color: '#101828', fontWeight: 600 }}>{pageTitle}</span>
            </div>

            {tabs.length > 0 && (
                <div style={{ display: 'flex', gap: 4, borderBottom: `1px solid ${BORDER}`, marginBottom: 20 }}>
                    {tabs.map(t => (
                        <div key={t.label} onClick={() => t.route && navigate(t.route)}
                            style={{ padding: '10px 16px', fontSize: 14, fontWeight: 600, cursor: t.route ? 'pointer' : 'default', color: t.active ? PURPLE : '#667085', borderBottom: t.active ? `2px solid ${PURPLE}` : '2px solid transparent' }}>
                            {t.label}
                        </div>
                    ))}
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: '#101828' }}>{pageTitle}</h1>
                    <p style={{ margin: '4px 0 0', fontSize: 13, color: '#667085' }}>Manage and publish this homepage section.</p>
                </div>
                <button onClick={addCard} style={{ display: 'flex', alignItems: 'center', gap: 6, background: PURPLE, color: '#fff', border: 'none', borderRadius: 8, padding: '10px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                ...
                <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 24, marginBottom: 28 }}>
                        <div style={{ flex: 1 }}>
                            ...
                            <input style={inputStyle} value={title} onChange={e => setTitle(e.target.value)} placeholder={defaultTitle} />
                        </div>
                    ...
                    <div style={{ border: '1px solid #EAECF0', borderRadius: 12, overflow: 'hidden' }}>
                        <div style={{ padding: '14px 20px', background: '#F9FAFB', borderBottom: '1px solid #EAECF0' }}>
                        ...
                        {cards.length === 0 && <div style={{ padding: 32, textAlign: 'center', color: '#98A2B3', fontSize: 13 }}>No cards yet. Click “{addLabel}”.</div>}
                        {cards.map((c, i) => (
                            <div key={c.id || `new-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderBottom: '1px solid #F2F4F7' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#344054', marginBottom: 4 }}>{nameLabel}</label>
                                    <input style={inputStyle} value={c.name} onChange={e => setCard(i, 'name', e.target.value)} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#344054', marginBottom: 4 }}>Link / URL</label>
                                    <input style={inputStyle} value={c.link} onChange={e => setCard(i, 'link', e.target.value)} placeholder="/shop/..." />
                                </div>
...
export default SectionCardsEditor;
